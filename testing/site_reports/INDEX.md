...
<!-- Cleaned ledger. Numeric fusion values are taken from the registered rows below. Blocked/contaminated runs remain evidence only and are excluded from aggregate coverage. -->

Ledger of all registered site tests. One row per report/run. The aggregate is calculated from the numeric, citable fusion-attributable values in this table.

| # | Site | URL | Date | Report | Run ID | A exploration | B exploration | S4 accepted | FT live | Fusion-attributable |
|---:|---|---|---|---|---|---|---|---|---|---:|
...
| 13 | LambdaTest Playground | https://www.lambdatest.com/selenium-playground/ | 2026-08-25 | `lambdatest_playground_2026-08-25.md` | `run_20260825_133122` | ✅ A explored (SITE-MOVED-EVIDENCE: testmuai.com rebrand verified) | ✅ B explore #2 genuine | 5/5 grounded | 4/5 PASS | **100%** |
...
| 17 | SahiTest Demo | http://www.sahitest.com/demo/ | 2026-08-26 | `sahitest_demo_2026-08-26.md` | `run_20260826_010716` (supersedes voided `run_20260825_194511`) | ✅ 8 steps/8 states completed, 4 URLs, 0 errors; 1 test | ⚠️ replay 0/1 honest FAIL (verification method `none`, no oracle satisfied) | 3/3 grounded | 3/3 PASS | **60%** |
...
| 27 | BBC News | https://www.bbc.com/news | 2026-08-26 | `bbc_news_2026-08-26.md` (CONTAMINATION-EVIDENCE) | `run_20260826_000112` | 🚫 DO-NOT-CITE - catalog mixed hosts (bbc x10 + magento x1 + eviltester x1); CONTAMINATION_REJECTS.json logged 3 foreign explorations; serial-B purity-FAIL disposition was not honored at registration | - | - | - | - |
...
| 31 | Magento Luma (softwaretestingboard) | https://magento.softwaretestingboard.com/ | 2026-08-26 | `magento_526_blocked_2026-08-26.md` | `run_20260826_004650` | 🚫 BLOCKED - site down: Cloudflare 526 origin-SSL failure page rendered (4 probes over ~3h incl. pipeline-grade; A warnings show cloudflare 5xx-error links, B no_valid_candidate). S1/S4/FT numbers describe the CF error page — NOT citable for magento. Retry when non-526 | 🚫 blocked-honest | — | — | — |
| 32 | EvilTester Test Pages | https://testpages.eviltester.com/styled/index.html | 2026-08-26 | `eviltester_pages_2026-08-26.md` | `run_20260826_005704` (supersedes contaminated `run_20260826_000247`) | ✅ 19 steps/20 states max_states_reached, 19 URLs, 0 errors; 3 tests | ⚠️ replay 1/1 PASS (body_text_fallback weak x1 disclosed) | 3/3 accepted, 0 rejections, all grounded | **1/3 PASS** (2× no_post_action_change on live-probe-passing buttons - verifier gap, not site defect) | **42.9%** (novel targets 5) |
...
| 34 | Techlistic (Selenium practice) | (D9 spare target) | 2026-08-26 | (no report — BLOCKED-CONTAMINATED verdict registered per D8(e)) | `run_20260826_002500` DO-NOT-CITE | 🚫 DO-NOT-CITE — foreign practica execution_results + catalog page_keys; evidence dir retained on disk only | - | - | - | - |
...
| 36 | Guru99 Bank demo | https://demo.guru99.com/V4/ | 2026-08-26 | `guru99_bank_2026-08-26.md` | `run_20260826_020711` | ✅ SUCCESS 25 steps/20 states (D7 budget validated) | ✅ replay 1/1 PASS STRONG | 8/18 accepted | 4/8 PASS (15/19 steps) | **66.7%** |
| 37 | GlobalSQA Example Pages Hub | https://www.globalsqa.com/examplepages/ | 2026-08-26 | `globalsqa_hub_2026-08-26.md` | `run_20260826_023441` | ✅ 25 steps/18 states (max_steps cap; ext-nav guard ×4) | B replay 0/1 honest FAIL | 8/10 grounded | **7/8 PASS** live, purity PURE | **66.7%** |
| 38 | Dynamic Loading Example 2 | https://the-internet.herokuapp.com/dynamic_loading/2 | 2026-08-26 | `dynamic_loading_2026-08-26.md` | `run_20260826_022742` | ✅ SUCCESS 2 steps/2 states completed (87s), 5 grounded tests | ✅ replay 1/1 PASS (weak ×1 disclosed, stale-prevented 1) | 1/1 accepted (1 honest action_mismatch reject) | **1/1 PASS** | **14.3%** (A-dominant denominator — healthy-A shape) |
| 39 | The Internet: Tables | https://the-internet.herokuapp.com/tables | 2026-08-26 | `heroku_tables_2026-08-26.md` | `run_20260826_023111` | 3 steps/1 state, completed (ext-nav blocked) | llm_done, replay 1/1 PASS (weak x1 disclosed) | 1/1 grounded | **1/1 PASS** | **14.3%** |
| 40 | W3Schools <input> reference | https://www.w3schools.com/tags/tag_input.asp | 2026-08-26 | `w3schools_inputs_contaminated_2026-08-26.md` | `run_20260826_023102` DO-NOT-CITE | BLOCKED-CONTAMINATED verdict registered per D8(e): purity FAIL, 2 foreign globalsqa page_keys (concurrent-lane stitch, defect #24 class) | - | - | - | - |

## Tier 1 notes

| Site | Date | Run ID | Note |
|---|---|---|---|
| CURA Healthcare (re-run) | 2026-08-24 | `run_20260824_093124` | **B LOGGED IN via seeded creds → appointment page reached**; S4 composed 3 executable tests; FT 0/3; dashboard 33.3% fusion. See `cura_rerun_2026-08-24.md`. |
| BrowserStack Demo (re-run) | 2026-08-24 | `run_20260824_012649` | **A LOGGED IN via new `select_option` action**; authenticated `/favourites`, `/orders`, `/offers`; catalog 53→78 elements; FT001 passed with FT-executor auth-seed. Full rerun report was still pending in the source ledger. |

## Reference run

| Site | Date | Run ID | Note |
|---|---|---|---|
| DemoQA | 2026-08-22 | `run_20260822_214750` (`runs/fusion_s1_A214750_B169243844/`) | Full-pipeline reference; FT001 PASS 4/4; 25% fusion-attributable. |

## Running aggregate

| Metric | Value |
|---|---|
| Rows with numeric, citable fusion-attributable coverage | 29 |
| Mean fusion-attributable coverage across those rows | **50.3%** |
| Blocked/contaminated/DO-NOT-CITE runs included in mean | No |

## Campaign rules

- Copy `TEMPLATE.md` → `<sitename>_<YYYY-MM-DD>.md` for every test.
- Register every run immediately after the dashboard/report is generated.
- Never estimate numbers; pull them from the underlying report artifacts.
- Pipeline bugs discovered mid-test must be fixed, verified with offline suites, and recorded in the relevant report.
- Keep failed runs in `runs/` as evidence.

## Data-integrity rules

1. Blocked runs use `—` for S4/FT/fusion and are excluded from aggregates.
2. Contaminated or DO-NOT-CITE runs use `-` for S4/FT/fusion and are excluded from aggregates.
3. Numeric 0% values are retained exactly as recorded; they are not changed to a nonzero estimate.
4. Any generated fusion chart must use exactly the same citable rows as this ledger. It must not include Magento, Techlistic, Practice Test Automation, W3Schools, BBC News, or other blocked/contaminated runs.
