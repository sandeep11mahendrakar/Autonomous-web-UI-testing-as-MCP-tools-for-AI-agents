...
if (!key || !url) { console.error('usage: node tier3_w1.cjs <key> <url>'); process.exit(1); }
...
function log(m) {
  const line = `[tier3w1 ${new Date().toISOString()}] ${m}`;
  console.log(line);
  fs.appendFileSync(path.join(__dirname, 'tier3_w1.log'), line + '\n');
}
