...
- Prefer assertions verifiable from the page after the action: URL change (only when justified), non-empty rendered body, field/state changes, absence of errors.
...
- Generate 5-10 test cases covering different element types (inputs, radio/checkbox groups, selects, links, buttons). Skip elements that are too ambiguous to test meaningfully rather than inventing behavior.
...
- "evidence": array of strings — the visual facts used (keep each item under ~15 words)
...
- "steps": array of { "action": "click"|"fill"|"navigate", "x": number|null, "y": number|null, "value": string }
  Use element centre coordinates for click/fill. Use null coordinates for navigate.
