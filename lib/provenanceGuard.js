...
function artifactBelongsToRun(artifact, manifestUrl) {
  ...
  if (!urls.length) {
    // AUDIT F4-05: legacy pass-through, but no longer SILENT - callers get an
    // explicit warn so url-less artifacts (provenance unverifiable) are visible.
    return { ok: true, via: 'no_url_fields', warn: 'artifact references no URL - provenance unverifiable' };
  }
